...
- FFZY 保存详情时按 `(source_key, source_item_id, episode_index)` 比较现有分集：标题和 URL 均未变化时保留原 `updated_at`；新增分集或标题/URL 实际变化时才写入本次时间；远端已删除的分集仍删除。例行详情刷新不得重写所有分集时间。
- 每个采集站条目只以当前最大 `episode_index` 的分集作为 Updates 候选；该分集的 `updated_at` 位于请求窗口内时，才表示当前最新分集在窗口内新增或变化。
- 没有分集变化但采集站条目时间变化时，不伪造番剧剧集更新；旧分集的 URL 修复也不把已过季条目重新推到 Updates。
- 一条 Bangumi 映射只有在其投影范围包含该当前最新源分集时才成为候选。
